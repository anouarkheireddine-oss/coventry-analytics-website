      ...
      <article className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white leading-tight mb-4">
            ...
            <span className="text-[#00d4ff]">The Real Numbers, {year}</span>
          </h1>
        ...
        {sweLondon && sweBest && sweBest.slug !== 'london' && (
          <div className="rounded-2xl border border-[#f59e0b30] bg-[#f59e0b08] p-6 mb-8">
            ...
            <p className="text-lg font-bold text-white leading-snug">
              A software engineer in <span className="text-[#22c55e]">{sweBest.city}</span> takes home{' '}
              <span className="text-[#22c55e]">{formatGBP(sweBest.disposable)}/month</span> disposable income —
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {ranked.map(({ city, slug, gross, netMonthly, costs, disposable }, i) => (
                        <tr key={slug} className={`border-b border-white/[0.03] ${slug === 'london' ? 'bg-[#f59e0b06]' : i === 0 ? 'bg-[#22c55e06]' : ''} hover:bg-white/[0.02] transition-colors`}>
                          <td className="px-3 py-2.5">
                            <Link href={`/salary/${role.slug}/${slug}`} className="font-semibold text-white/80 hover:text-white transition-colors">
                              {city}
                              {i === 0 && <span className="ml-1.5 text-[9px] font-bold text-[#22c55e] uppercase">best</span>}
                          ...
                          <td className="px-3 py-2.5 text-[#00d4ff] font-semibold">{formatGBP(netMonthly)}</td>
                          <td className="px-3 py-2.5 text-white/35 text-xs">{formatGBP(costs)}</td>
                          <td className="px-3 py-2.5">
                            <span className={`font-bold ${i === 0 ? 'text-[#22c55e]' : slug === 'london' ? 'text-[#f59e0b]' : 'text-white/60'}`}>
        ...
        {/* Verdict */}
        <div className="rounded-2xl border border-[#00d4ff20] bg-[#00d4ff06] p-6 mb-8">
