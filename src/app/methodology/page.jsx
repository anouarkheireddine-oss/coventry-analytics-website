    ...
    <main className="min-h-screen bg-[#f8f7f5] text-gray-900">
      <article className="max-w-3xl mx-auto px-4 py-10">
        <nav className="flex items-center gap-2 text-xs text-black/30 mb-8">
          ...
          <span>›</span>
          <span className="text-black/60">Methodology</span>
        </nav>

        <div className="mb-10">
          <span className="px-2.5 py-1 rounded-full bg-black/[0.04] border border-black/[0.08] text-[11px] font-semibold text-black/50 uppercase tracking-wider">Transparency</span>
        ...
        <section className="mb-10">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Salary Data</h2>

          <div className="space-y-4 text-black/60 text-[15px] leading-relaxed">
            <p>
            ...
            <div className="rounded-xl border border-black/[0.07] overflow-hidden mt-2">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-black/[0.07] bg-black/[0.025]">
                    {['Level', 'Multiplier', 'Example (£65k mid)'].map(h => (
                      <th key={h} className="px-4 py-2.5 text-left text-[10px] font-semibold text-black/30 uppercase tracking-wider">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={level} className="border-b border-black/[0.04]">
                      <td className="px-4 py-2.5 text-black/70 font-medium">{level}</td>
                      <td className="px-4 py-2.5 text-black/45">{mult}</td>
                      <td className="px-4 py-2.5 text-black/45">{ex}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* Income tax */}
        <section className="mb-10">
          <h2 className="text-xl font-bold text-gray-900 mb-2">Income Tax (2025/26)</h2>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {INCOME_TAX_BANDS.map(({ band, range, rate, note }) => (
                  <tr key={band} className="border-b border-black/[0.04]">
                    <td className="px-4 py-2.5 text-black/70 font-medium">{band}</td>
                    <td className="px-4 py-2.5 text-black/45">{range}</td>
                    <td className="px-4 py-2.5 font-bold text-[#818cf8]">{rate}</td>
                    <td className="px-4 py-2.5 text-black/35 text-xs">{note}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* NI */}
        <section className="mb-10">
          <h2 className="text-xl font-bold text-gray-900 mb-2">National Insurance (2025/26)</h2>
          <p className="text-black/55 text-[15px] leading-relaxed mb-4">
        ...
        <section className="mb-10">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Pension and Student Loan</h2>
          <div className="space-y-4 text-black/60 text-[15px] leading-relaxed">
            <p>
              <strong className="text-black/80">Pension:</strong> Default assumption is a 5% employee contribution via salary sacrifice.
            ...
            <p>
              <strong className="text-black/80">Student Loan:</strong> Default is Plan 2 (post-2012 undergraduates in England/Wales).
        ...
        {/* Living costs */}
        <section className="mb-10">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Living Cost Estimates</h2>
          <div className="space-y-4 text-black/60 text-[15px] leading-relaxed">
            <p>
            ...
            <ul className="space-y-2">
              {[
                ...
                <li key={item} className="flex gap-3">
                  ...
                  <span><strong className="text-black/70">{item}:</strong> {detail}</span>
                </li>
              ))}
            </ul>
            <p>
        ...
        {/* Limitations */}
        <section className="mb-10">
          <h2 className="text-xl font-bold text-gray-900 mb-4">What These Numbers Don&apos;t Cover</h2>
