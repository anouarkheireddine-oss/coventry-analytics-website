...
import { QUIZ_STEPS, calculateScores, getMaturityLevel } from '@/lib/scoring'
...
export default function BusinessHealthScoreTool() {
  ...
  if (stage === STAGES.RESULTS && scores) {
    const maturity = getMaturityLevel(scores.overallScore)
    return (
      <>
        <ResultsDisplay
  ...
  return (
    <>
      <EmailCaptureModal
        ...
        source="bhs-results"
        score={scores?.overallScore}
        maturityLevel={scores ? getMaturityLevel(scores.overallScore).level : ''}
      />
