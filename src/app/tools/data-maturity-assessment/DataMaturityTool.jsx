...
import { DATA_QUIZ_STEPS, calculateDataScore, getDataMaturityLevel, getDataMaturityRecommendations } from '@/lib/dataMaturity'
...
export default function DataMaturityTool() {
  ...
  const step = DATA_QUIZ_STEPS[currentStep]
  const totalSteps = DATA_QUIZ_STEPS.length
  ...
  const handleNext = () => {
    if (currentStep < totalSteps - 1) {
      ...
      const computed = calculateDataScore(answers)
      setScores(computed)
  ...
  if (stage === 'results' && scores) {
    const maturity = getDataMaturityLevel(scores.overallScore)
    const recs = getDataMaturityRecommendations(scores.dimensionScores)
    const dimensionList = DATA_QUIZ_STEPS.map(s => ({ id: s.id, title: s.title, score: scores.dimensionScores[s.id] || 0 }))
    return (
      <>
        <GenericToolResults
          ...
          toolTitle="Data Maturity Assessment"
          scoreLabel="Your Data Maturity Score"
          pdfFilename={`data-maturity-${scores.overallScore}.pdf`}
        ...
        <EmailCaptureModal isOpen={emailModalOpen} onClose={handleSkip} onSubmit={handleEmailSubmit}
          source="data-maturity-download" score={scores.overallScore} maturityLevel={maturity.level} />
      </>
    )
  }
  ...
  return (
    <>
      <EmailCaptureModal isOpen={emailModalOpen} onClose={handleSkip} onSubmit={handleEmailSubmit}
        source="data-maturity-results" score={scores?.overallScore} maturityLevel={scores ? getDataMaturityLevel(scores.overallScore).level : ''} />
      <div>
        <div className="mb-8">
