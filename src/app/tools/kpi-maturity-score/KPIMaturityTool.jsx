...
import { KPI_QUIZ_STEPS, calculateKPIScore, getKPIMaturityLevel, getKPIRecommendations } from '@/lib/kpiMaturity'
...
export default function KPIMaturityTool() {
  ...
  const step = KPI_QUIZ_STEPS[currentStep]
  const totalSteps = KPI_QUIZ_STEPS.length
  ...
  const handleNext = () => {
    if (currentStep < totalSteps - 1) {
      ...
      const computed = calculateKPIScore(answers)
      setScores(computed)
  ...
  if (stage === 'results' && scores) {
    const maturity = getKPIMaturityLevel(scores.overallScore)
    const recs = getKPIRecommendations(scores.dimensionScores)
    const dimensionList = KPI_QUIZ_STEPS.map(s => ({ id: s.id, title: s.title, score: scores.dimensionScores[s.id] || 0 }))
    return (
      <>
        <GenericToolResults
          ...
          toolTitle="KPI Maturity Score"
          scoreLabel="Your KPI Maturity Score"
          pdfFilename={`kpi-maturity-${scores.overallScore}.pdf`}
        ...
        <EmailCaptureModal isOpen={emailModalOpen} onClose={handleSkip} onSubmit={handleEmailSubmit}
          source="kpi-maturity-download" score={scores.overallScore} maturityLevel={maturity.level} />
      </>
    )
  }
  ...
  return (
    <>
      <EmailCaptureModal isOpen={emailModalOpen} onClose={handleSkip} onSubmit={handleEmailSubmit}
        source="kpi-maturity-results" score={scores?.overallScore} maturityLevel={scores ? getKPIMaturityLevel(scores.overallScore).level : ''} />
      <div>
        <div className="mb-8">
