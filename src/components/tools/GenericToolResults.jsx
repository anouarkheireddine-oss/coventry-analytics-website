  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const addInnerPageChrome = () => {
        ...
        doc.text(toolTitle, pageW - margin, 9, { align: 'right' })
      ...
      // Clip title if long
      const titleText = toolTitle.length > 30 ? toolTitle.toUpperCase() : toolTitle.toUpperCase()
      T(titleText, margin, 60, { size: 18, color: [255, 255, 255], bold: true })
      ...
      for (let i = 0; i < 80; i++) {
        const a1 = startAngle + (i / 80) * (endAngle - startAngle)
        const a2 = startAngle + ((i + 1) / 80) * (endAngle - startAngle)
      ...
      T(toolTitle, margin + contentW / 2, metaY + 15, { size: 8, color: [226, 232, 240], bold: true, align: 'center' })
      ...
      T(`${dimensionList.length}`, margin + 6, metaY + 29, { size: 8.5, color: [226, 232, 240], bold: true })
      T('Questions', margin + contentW / 2, metaY + 22, { size: 7, color: [71, 85, 105], align: 'center' })
      T(`${dimensionList.length * 2}`, margin + contentW / 2, metaY + 29, { size: 8.5, color: [226, 232, 240], bold: true, align: 'center' })
      ...
      // ── PAGE 2: EXECUTIVE SUMMARY + COST OF INACTION + SCORES ──────
      addPage()
      ...
      const interp = overallScore >= 70
        ? `A score of ${overallScore}/100 places your business in the upper tier of UK SMEs assessed by Coventry Analytics. Your foundations are strong. The recommendations in this report will help you reach best-in-class performance and protect your competitive advantage.`
        : overallScore >= 40
        ? `A score of ${overallScore}/100 is in line with the median for UK SMEs at your stage. You have genuine strengths alongside gaps that are currently costing you time, money, and competitive position. The recommendations below are ordered by impact.`
        : `A score of ${overallScore}/100 indicates foundational gaps that are currently limiting growth and increasing operational risk. The highest-impact improvements can typically be made within 90 days without large capital outlay.`
      ...
      y += 3
      // Score distribution — 2-line labels per column to avoid collision
      checkPage(22)
      ...
      const distCols = [
        { l1: 'Strong', l2: '70+', val: strongCount, color: [34, 197, 94] },
        { l1: 'Developing', l2: '50-69', val: devCount, color: [59, 130, 246] },
        { l1: 'Needs Work', l2: '30-49', val: weakCount, color: [245, 158, 11] },
        { l1: 'Critical', l2: 'Below 30', val: critCount, color: [239, 68, 68] },
      ]
      ...
      if (strengthDims.length === 0) {
        ...
        strengthDims.forEach(({ title, score: s }) => {
          ...
          // Green indicator block — no emoji
      ...
      // Plain text priority labels — no emoji (jsPDF Helvetica is Latin-1 only)
      const priorityMeta = {
      ...
      ;[
        ...
        `Identify your highest-priority gap (${weakDims[0]?.title || 'your lowest scoring area'})`,
      ...
      ;[
        ...
        'Run this assessment again to see your dimension scores move',
