  ...
  return (
    <div className="rounded-2xl border border-black/[0.07] bg-black/[0.02] p-5">
      <input
        ...
        placeholder={label}
        className="w-full bg-transparent text-base font-bold text-gray-900 placeholder-white/20 border-b border-black/[0.08] pb-2 mb-4 focus:outline-none focus:border-[#818cf840]"
      />
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={k}>
            <label className="text-[11px] text-black/35 font-medium block mb-1">{l}</label>
            <div className="relative">
              {prefix && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-black/35 text-sm">{prefix}</span>}
              <input
                type="number"
                value={value[k]}
                onChange={e => set(k, +e.target.value || 0)}
                step={step}
                className={`w-full bg-black/[0.04] border border-black/[0.08] rounded-xl ${prefix ? 'pl-7' : 'pl-3'} pr-3 py-2 text-sm font-semibold text-gray-900 focus:outline-none focus:border-[#818cf840]`}
              />
              {suffix && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-black/35 text-sm">{suffix}</span>}
            </div>
          </div>
        ))}
      </div>
      <label className="flex items-center gap-2 mt-3 cursor-pointer">
        ...
        <span className="text-xs text-black/40">Remote / hybrid (saves ~£2,400/yr commute)</span>
      </label>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      {(a.salary > 0 || b.salary > 0) && (
        <div className={`rounded-2xl border p-5 ${winner === 'a' ? 'border-[#34d39930] bg-[#34d39908]' : 'border-[#818cf830] bg-[#818cf808]'}`}>
          ...
          <p className="text-xs text-black/40">That&apos;s {formatGBP(diff * 3)} over 3 years · {formatGBP(diff * 5)} over 5 years</p>
        </div>
      )}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-black/[0.07] bg-black/[0.025]">
              <th className="px-4 py-3 text-left text-[11px] font-semibold text-black/30 uppercase tracking-wider">Component</th>
              <th className="px-4 py-3 text-right text-[11px] font-semibold text-black/30 uppercase tracking-wider">{a.label || 'Offer A'}</th>
              <th className="px-4 py-3 text-right text-[11px] font-semibold text-black/30 uppercase tracking-wider">{b.label || 'Offer B'}</th>
            </tr>
          </thead>
          <tbody>
            {[
              ...
              <tr key={label} className={`border-b border-black/[0.04] ${total ? 'bg-black/[0.03] font-bold' : highlight ? 'bg-black/[0.02]' : ''}`}>
                <td className={`px-4 py-3 ${total ? 'text-gray-900 font-bold' : 'text-black/60'}`}>{label}</td>
                <td className={`px-4 py-3 text-right ${winner === 'a' && total ? 'text-[#34d399] font-bold text-base' : 'text-black/70'}`}>
                  {fmt ? formatGBP(va) : va}
                </td>
                <td className={`px-4 py-3 text-right ${winner === 'b' && total ? 'text-[#818cf8] font-bold text-base' : 'text-black/70'}`}>
                  {fmt ? formatGBP(vb) : vb}
                </td>
              </tr>
            ))}
      ...
      <p className="text-[11px] text-black/20">Net take-home uses 2025/26 UK tax rates, Plan 2 student loan, 5% employee pension. Bonus taxed at 40% marginal rate estimate. Not financial advice.</p>
    </div>
  );
}
