...
function ResultCard({ result, highlight }) {
  return (
    <div className={`rounded-2xl border p-5 ${highlight ? 'border-[#00d4ff30] bg-[#00d4ff06]' : 'border-white/[0.07] bg-white/[0.02]'}`}>
      <p className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-3">{result.label}</p>
      <p className="text-3xl font-extrabold text-[#00d4ff] mb-0.5">{formatGBP(result.netMonthly ?? result.netAnnual / 12)}</p>
        ...
        {[{ v: 'dayRate', l: 'I know my day rate' }, { v: 'salary', l: 'I know my salary' }].map(({ v, l }) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
              mode === v ? 'bg-[#00d4ff20] border border-[#00d4ff40] text-[#00d4ff]' : 'border border-white/[0.08] text-white/40 hover:text-white/60'
            }`}
          >
            {l}
          </button>
      ...
      <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input type="number" value={days} onChange={e => setDays(+e.target.value || 0)} step={5} min={100} max={260}
              className="w-full bg-white/[0.06] border border-white/[0.10] rounded-xl px-3 py-2.5 text-sm font-semibold text-white focus:outline-none focus:border-[#00d4ff40]" />
        ...
        {outsideVsPerm !== 0 && (
          <div className={`mt-5 px-4 py-3 rounded-xl border text-sm font-semibold ${
            outsideVsPerm > 0 ? 'border-[#22c55e25] bg-[#22c55e08] text-[#22c55e]' : 'border-[#ef444425] bg-[#ef444408] text-[#ef4444]'
          }`}>
