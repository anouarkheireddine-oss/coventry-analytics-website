...
function ResultCard({ result, highlight }) {
  return (
    <div className={`rounded-2xl border p-5 ${highlight ? 'border-[#818cf830] bg-[#818cf806]' : 'border-black/[0.07] bg-black/[0.02]'}`}>
      <p className="text-xs font-semibold text-black/40 uppercase tracking-wider mb-3">{result.label}</p>
      <p className="text-3xl font-extrabold text-[#818cf8] mb-0.5">{formatGBP(result.netMonthly ?? result.netAnnual / 12)}</p>
      <p className="text-xs text-black/30 mb-4">/month take-home</p>
      <p className="text-sm font-semibold text-black/70 mb-3">{formatGBP(result.netAnnual)}/year</p>
      ...
      <div className="mt-3 pt-3 border-t border-black/[0.06]">
        <p className="text-xs text-black/30">Effective tax rate: <span className="text-black/60 font-semibold">{result.effectiveRate}%</span></p>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-6">
        ...
        {[{ v: 'dayRate', l: 'I know my day rate' }, { v: 'salary', l: 'I know my salary' }].map(({ v, l }) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
              mode === v ? 'bg-[#818cf820] border border-[#818cf840] text-[#818cf8]' : 'border border-black/[0.08] text-black/40 hover:text-black/60'
            }`}
          >
            {l}
          </button>
      ...
      <div className="rounded-2xl border border-black/[0.07] bg-black/[0.02] p-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {mode === 'dayRate' ? (
            <div>
              <label className="text-xs text-black/40 font-medium block mb-1.5">Day Rate</label>
              ...
              <p className="text-[10px] text-black/25 mt-1">≈ {formatGBP(equivalentSalary)} perm equivalent</p>
            </div>
          ) : (
            <div>
              <label className="text-xs text-black/40 font-medium block mb-1.5">Permanent Salary</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-black/40 text-sm font-semibold">£</span>
                <input type="number" value={salary} onChange={e => setSalary(+e.target.value || 0)} step={1000}
                  className="w-full bg-black/[0.04] border border-black/[0.10] rounded-xl pl-7 pr-3 py-2.5 text-sm font-semibold text-gray-900 focus:outline-none focus:border-[#818cf840]" />
              </div>
              <p className="text-[10px] text-black/25 mt-1">≈ {formatGBP(equivalentDayRate)}/day contract equivalent</p>
            </div>
          )}

          <div>
            <label className="text-xs text-black/40 font-medium block mb-1.5">Billing Days / Year</label>
            <input type="number" value={days} onChange={e => setDays(+e.target.value || 0)} step={5} min={100} max={260}
              className="w-full bg-black/[0.04] border border-black/[0.10] rounded-xl px-3 py-2.5 text-sm font-semibold text-gray-900 focus:outline-none focus:border-[#818cf840]" />
            <p className="text-[10px] text-black/25 mt-1">Default: {DEFAULT_BILLING_DAYS} (incl. holiday)</p>
          </div>

          <div>
            <label className="text-xs text-black/40 font-medium block mb-1.5">Ltd Co. Overheads / Year</label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-black/40 text-sm font-semibold">£</span>
              <input type="number" value={overheads} onChange={e => setOverheads(+e.target.value || 0)} step={500}
                className="w-full bg-black/[0.04] border border-black/[0.10] rounded-xl pl-7 pr-3 py-2.5 text-sm font-semibold text-gray-900 focus:outline-none focus:border-[#818cf840]" />
            </div>
            <p className="text-[10px] text-black/25 mt-1">Accountant + insurance + tools</p>
          </div>

          {mode === 'dayRate' ? (
            <div>
              <label className="text-xs text-black/40 font-medium block mb-1.5">Perm Comparison Salary</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-black/40 text-sm font-semibold">£</span>
                <input type="number" value={salary} onChange={e => setSalary(+e.target.value || 0)} step={1000}
                  className="w-full bg-black/[0.04] border border-black/[0.10] rounded-xl pl-7 pr-3 py-2.5 text-sm font-semibold text-gray-900 focus:outline-none focus:border-[#818cf840]" />
            ...
            <div>
              <label className="text-xs text-black/40 font-medium block mb-1.5">Contract Day Rate</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-black/40 text-sm font-semibold">£</span>
                <input type="number" value={dayRate} onChange={e => setDayRate(+e.target.value || 0)} step={50}
                  className="w-full bg-black/[0.04] border border-black/[0.10] rounded-xl pl-7 pr-3 py-2.5 text-sm font-semibold text-gray-900 focus:outline-none focus:border-[#818cf840]" />
